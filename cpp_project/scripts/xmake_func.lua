...
on_load(function(target)
    ...
    target:add("links", "lc-core", "lc-gui", "lc-dsl", "lc-runtime", "lc-vstl", "lc-ast", "lc-ext-eastl", "lc-ext-spdlog")
end)
rule_end()
rule("copy_dll")
after_build(function(target)
	...
	local dst_dir = path.join(os.projectdir(), "../Assets/Plugins")
	os.cp(path.join(lc_path, bin_dir, "*.dll"), dst_dir)
	os.cp(path.join(os.projectdir(), bin_dir, "*.dll"), dst_dir)
end)
rule_end()

-- In-case of submod, when there is override rules, do not overload
if _config_rules == nil then
	_config_rules = {"lc_unity_basic_settings"}
end
