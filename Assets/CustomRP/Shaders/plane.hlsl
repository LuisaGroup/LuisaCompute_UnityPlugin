    ...
    bool BoxIntersectBool(float3 extent, float3x3 boxLocalToWorld, float3 position, float4 planes[6])
    {
        for(uint i = 0; i < 6; ++i)
        {
            float4 plane = planes[i];
            float3 absNormal = abs(mul(plane.xyz, boxLocalToWorld));
            if((dot(position, plane.xyz) - dot(absNormal, extent)) < -plane.w) return false;
        }
        return true;
    }
